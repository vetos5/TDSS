    ...
    <div class="p-5">
      ...
      <div class="flex items-center justify-between mb-4">
        ...
        <button @click="emit('toggle-theme')"
                ...
                :class="dark ? 'border-slate-600 hover:bg-slate-700' : 'border-slate-300 hover:bg-slate-100'">
          {{ dark ? '☀️' : '🌙' }}
        </button>
      </div>

      <hr :class="dark ? 'border-slate-700' : 'border-slate-200'" class="mb-4" />
      ...
      <div class="mb-4">
        ...
        <div class="space-y-3">
          <div>
            <label class="text-xs" :class="dark ? 'text-slate-300' : 'text-slate-600'">Construction Cost — Minimize</label>
            <input type="range" v-model.number="dss.weights.construction_cost_mln" min="0" max="100" step="5"
                   class="w-full h-1.5 rounded-lg appearance-none cursor-pointer accent-teal-600" />
            <span class="text-xs font-medium" :class="dark ? 'text-slate-300' : 'text-slate-600'">{{ dss.weights.construction_cost_mln }}%</span>
          </div>
          <div>
            <label class="text-xs" :class="dark ? 'text-slate-300' : 'text-slate-600'">Land Area — Minimize</label>
            <input type="range" v-model.number="dss.weights.land_area_hectares" min="0" max="100" step="5"
                   class="w-full h-1.5 rounded-lg appearance-none cursor-pointer accent-orange-600" />
            <span class="text-xs font-medium" :class="dark ? 'text-slate-300' : 'text-slate-600'">{{ dss.weights.land_area_hectares }}%</span>
          </div>
          <div>
            <label class="text-xs" :class="dark ? 'text-slate-300' : 'text-slate-600'">Throughput / Capacity — Maximize</label>
            <input type="range" v-model.number="dss.weights.throughput_vph" min="0" max="100" step="5"
                   class="w-full h-1.5 rounded-lg appearance-none cursor-pointer accent-violet-600" />
            <span class="text-xs font-medium" :class="dark ? 'text-slate-300' : 'text-slate-600'">{{ dss.weights.throughput_vph }}%</span>
          </div>
          <div>
            <label class="text-xs" :class="dark ? 'text-slate-300' : 'text-slate-600'">Safety Index — Maximize</label>
            <input type="range" v-model.number="dss.weights.safety_index" min="0" max="100" step="5"
                   class="w-full h-1.5 rounded-lg appearance-none cursor-pointer accent-blue-600" />
            <span class="text-xs font-medium" :class="dark ? 'text-slate-300' : 'text-slate-600'">{{ dss.weights.safety_index }}%</span>
          </div>
        </div>
      </div>
      ...
      <div class="mb-4">
        ...
        <div class="space-y-2">
          <div>
            <label class="text-xs" :class="dark ? 'text-slate-300' : 'text-slate-600'">Design Speed (km/h)</label>
            <select v-model.number="dss.params.design_speed"
                    class="w-full rounded-lg border px-3 py-1.5 text-sm"
                    :class="dark ? 'bg-slate-700 border-slate-600 text-slate-200' : 'bg-slate-50 border-slate-300'">
              <option v-for="s in [60,80,100,120,140]" :key="s" :value="s">{{ s }}</option>
            </select>
          </div>
          <div>
            <label class="text-xs" :class="dark ? 'text-slate-300' : 'text-slate-600'">AADT</label>
            <input type="number" v-model.number="dss.params.aadt" min="1000" max="200000" step="5000"
                   class="w-full rounded-lg border px-3 py-1.5 text-sm"
                   :class="dark ? 'bg-slate-700 border-slate-600 text-slate-200' : 'bg-slate-50 border-slate-300'" />
          </div>
          <div>
            <label class="text-xs" :class="dark ? 'text-slate-300' : 'text-slate-600'">Peak Hour Factor (%)</label>
            <input type="range" v-model.number="dss.params.peak_factor" min="5" max="20" step="1"
                   class="w-full h-1.5 rounded-lg appearance-none cursor-pointer accent-teal-600" />
            <span class="text-xs" :class="dark ? 'text-slate-300' : 'text-slate-600'">{{ dss.params.peak_factor }}%</span>
          </div>
          <div>
            <label class="text-xs" :class="dark ? 'text-slate-300' : 'text-slate-600'">Lanes / Direction</label>
          ...
          <div>
            <label class="text-xs" :class="dark ? 'text-slate-300' : 'text-slate-600'">Budget (M USD)</label>
            <input type="number" v-model.number="dss.params.budget" min="0" max="500" step="5"
                   class="w-full rounded-lg border px-3 py-1.5 text-sm"
                   :class="dark ? 'bg-slate-700 border-slate-600 text-slate-200' : 'bg-slate-50 border-slate-300'" />
          </div>
          <div>
            <label class="text-xs" :class="dark ? 'text-slate-300' : 'text-slate-600'">Available Land (ha)</label>
            <input type="number" v-model.number="dss.params.land_limit" min="0" max="100" step="1"
                   class="w-full rounded-lg border px-3 py-1.5 text-sm"
                   :class="dark ? 'bg-slate-700 border-slate-600 text-slate-200' : 'bg-slate-50 border-slate-300'" />
          </div>
          <div>
            <label class="text-xs" :class="dark ? 'text-slate-300' : 'text-slate-600'">Terrain Type</label>
          ...
          <div>
            <label class="text-xs" :class="dark ? 'text-slate-300' : 'text-slate-600'">Environmental Sensitivity</label>
      ...
      <div class="space-y-1.5 mb-3">
        <div v-for="(label, key) in CRITERION_LABELS" :key="key"
          ...
          <span class="w-8 text-right">{{ normW[key]?.toFixed(2) }}</span>
        </div>
      </div>

      <p class="text-xs" :class="Math.abs(rawTotal - 100) <= 1
        ? (dark ? 'text-teal-400' : 'text-teal-700')
        : 'text-orange-500'">
        Raw slider total: {{ rawTotal }}%
      </p>

      <hr :class="dark ? 'border-slate-700' : 'border-slate-200'" class="my-4" />
